  ...
  head: () => ({
    meta: [
      { title: "Find Your Style — MAISON" },
      {
        name: "description",
        content:
          "Tell us your aesthetic, occasion and budget. We'll surface a hand-curated edit — with no algorithms involved.",
      },
    ],
  }),
    ...
    <div style={{ padding: "64px 24px 80px" }}>
      ...
      >
        <Step
          ...
          options={styles.map((s) => ({ key: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))}
          value={aesthetic}
          onChange={setAesthetic}
        />
        <Step
          title="The occasion"
          options={occasions.map((s) => ({
            key: s,
        ...
        <Step
          title="Your budget"
          options={budgets.map((b) => ({ key: b.key, label: b.label }))}
          value={budget}
          onChange={setBudget}
        />
        ...
        <button
          ...
          style={{
            ...
            maxWidth: 400,
            padding: "16px 0",
            background: canShow ? "var(--gold)" : "#D9D2BD",
            color: "#fff",
            fontFamily: "'Inter', sans-serif",
            fontWeight: 500,
            fontSize: 13,
      ...
      <div ref={resultsRef} style={{ maxWidth: 1280, margin: "80px auto 0" }}>
        {results && (
          <>
            ...
            >
              {results.map((p) => (
                <ProductCard key={p.id} product={p} source="style-match" />
              ))}
            </div>
            <p
              style={{
            ...
            >
              These pieces were selected based on your stated preferences only. No browsing history,
