    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))" }}>
        ...
        <div style={{ padding: 48 }}>
          ...
          <div style={{ marginTop: 16, fontFamily: "'Inter', sans-serif", fontWeight: 500, fontSize: 26 }}>{formatINR(product.price)}</div>
          <div style={{ height: 1, background: "var(--border)", margin: "24px 0" }} />
          <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300, fontSize: 16, lineHeight: 1.8, color: "var(--gray)" }}>
            {product.description}
          </p>

          {product.category === "apparel" && (
            <div style={{ marginTop: 28 }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gray)", marginBottom: 10 }}>Size</div>
          ...
          <div style={{ marginTop: 28, display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", border: "1px solid var(--border)" }}>
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} style={{ padding: "10px 16px", fontSize: 16 }}>−</button>
              <span style={{ padding: "0 12px", fontFamily: "'Inter', sans-serif", fontWeight: 500, fontSize: 16 }}>{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} style={{ padding: "10px 16px", fontSize: 16 }}>+</button>
            </div>
          </div>

          <button onClick={handleAdd} style={{
            marginTop: 20, width: "100%", height: 56,
            background: "var(--black)", color: "#fff",
            fontFamily: "'Inter', sans-serif", fontWeight: 500, fontSize: 13,
            textTransform: "uppercase", letterSpacing: "0.12em",
          }}>{added ? "Added to Bag ✓" : "Add to Bag"}</button>

          <div style={{ marginTop: 12, textAlign: "center", fontFamily: "'Inter', sans-serif", fontWeight: 300, fontSize: 12, color: "var(--gray)" }}>
      ...
      {related.length > 0 && (
        <div style={{ maxWidth: 1280, margin: "80px auto 0", padding: "0 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 32 }}>
            {related.map((p) => <ProductCard key={p.id} product={p} source="category-browse" />)}
          </div>
        </div>
      )}
    </div>
